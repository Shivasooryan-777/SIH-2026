...
def run_historical_decision_backtest(
    ...
    Delegates to consolidated ml.backtest.run_decision_backtest to prevent duplicate code.
    Enforces candidate horizons N in {7, 14} days and Challenge 10 sanity check (< 25% savings).
    """
    from ml.backtest import run_decision_backtest
    return run_decision_backtest(db=db, oof_csv_path=oof_csv_path, candidate_horizons=CANDIDATE_HORIZONS)
